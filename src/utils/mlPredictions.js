...
export const loadMLPredictions = (games) => {
  ...
  games.forEach(game => {
    ...
    const homeAbbrev = teamNameToAbbrev[homeTeam] || homeTeam;
    const awayAbbrev = teamNameToAbbrev[awayTeam] || awayTeam;

    // Find matching prediction using abbreviations
    const prediction = mlPredictionsAll.find(
      pred => pred.home_team === homeAbbrev && pred.away_team === awayAbbrev
    );
