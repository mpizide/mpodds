...
import { getCFBOdds } from '../services/oddsAPI';
...
const CFBPage = () => {
  ...
  // Use imported CFB logo functions
  const getTeamLogo = getCFBTeamLogo;
  ...
  const fetchGames = useCallback(async (forceRefresh = false) => {
    try {
      setLoading(true);
      const response = await getCFBOdds(forceRefresh);
      const allData = response.data;

      // Filter for top 25 teams AND games with betting markets
      const top25Data = allData.filter(game => {
        if (!isTop25Game(game.home_team, game.away_team)) return false;

        // Check if game has spreads or moneylines
        const hasSpread = game.bookmakers?.some(book =>
          book.markets?.some(m => m.key === 'spreads')
        );
        const hasMoneyline = game.bookmakers?.some(book =>
          book.markets?.some(m => m.key === 'h2h')
        );

        return hasSpread || hasMoneyline;
      });

      setAllGames(top25Data);
      const data = top25Data;
                    ...
                    <div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        ...
                        <div style={{
                          ...
                          <div>
                            <div style={{ fontSize: '13px', fontWeight: '700', color: theme.text }}>
                              {getTeamLogo(homeTeam) ? getTeamShortName(homeTeam) : homeTeam}
                            </div>
                            {getTeamRanking(homeTeam) && (
                              <div style={{ marginTop: '2px' }}>
                                <span style={{
                                  ...
                                  Rank #{getTeamRanking(homeTeam)}
                                </span>
                              </div>
                            )}
                            {homeBestML && awayBestML && (
                              <div style={{ fontSize: '10px', color: homeBestML.odds < awayBestML.odds ? '#22c55e' : theme.textSecondary, marginTop: '2px' }}>
                        ...
                        <div style={{
                          ...
                          <div>
                            <div style={{ fontSize: '13px', fontWeight: '700', color: theme.text }}>
                              {getTeamLogo(awayTeam) ? getTeamShortName(awayTeam) : awayTeam}
                            </div>
                            {getTeamRanking(awayTeam) && (
                              <div style={{ marginTop: '2px' }}>
                                <span style={{
                                  background: '#fbbf24',
                                  color: '#000',
                                  ...
                                  Rank #{getTeamRanking(awayTeam)}
                                </span>
                              </div>
                            )}
                            {homeBestML && awayBestML && (
                              <div style={{ fontSize: '10px', color: awayBestML.odds < homeBestML.odds ? '#22c55e' : theme.textSecondary, marginTop: '2px' }}>
