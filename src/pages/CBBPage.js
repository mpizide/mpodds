...
const CBBPage = () => {
  ...
  const getTeamLogo = (teamName) => {
    ...
    // Most college teams follow format like "Duke Blue Devils" or "North Carolina Tar Heels"
    // ESPN uses team IDs, but we'll create a mapping for common teams
    ...
    // Fallback: try to extract abbreviation from team name
    const parts = teamName.split(' ');
    if (parts.length > 0) {
      const firstWord = parts[0].toLowerCase();
      return `https://a.espncdn.com/i/teamlogos/ncaa/500/${firstWord}.png`;
    }
  ...
  const getTeamShortName = (teamName) => {
    ...
    // Fallback: extract first word or acronym
    const parts = teamName.split(' ');
    if (parts.length === 1) {
      return teamName.toUpperCase().substring(0, 4);
    }

    // Use first word for single-word teams, or create acronym
    return parts[0].toUpperCase().substring(0, 4);
  };
