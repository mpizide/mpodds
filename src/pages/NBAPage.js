...
const NBAPage = () => {
  ...
  const [unitSize, setUnitSize] = useState(() => {
    const saved = localStorage.getItem('nba_unitSize');
  ...
  useEffect(() => {
    localStorage.setItem('nba_unitSize', unitSize.toString());
  }, [unitSize]);
  ...
  const groupGamesByDate = (games) => {
    ...
    twoDaysFromNow.setHours(23, 59, 59, 999); // End of day 2
  ...
  const fetchGames = useCallback(async (forceRefresh = false) => {
    try {
      setLoading(true);
      const response = await getNBAOdds(forceRefresh);
      ...
      const mlPredictions = loadNBAMLPredictions(data);
  ...
  const savePickOfDay = (game, type, team, odds, points, prediction, ev) => {
    try {
      ...
      const newPick = {
        ...
        odds: odds,
        bookmaker: 'Best Available', // You can track specific bookmaker if needed
        ev: ev,
              ...
              return (
                ...
                >
                  <div style={{
                    ...
                    <button
                      onClick={() => navigate(`/nba/props/${game.id}`)}
                      style={{
                        padding: '8px 16px',
                        background: '#3b82f6',
                        color: 'white',
